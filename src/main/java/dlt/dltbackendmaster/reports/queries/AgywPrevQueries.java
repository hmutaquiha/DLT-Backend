...
public class AgywPrevQueries {
	...
	public static final String AGYW_PREV = "select id, beneficiary_id, nui, current_age, date_of_birth, district_id, vblt_is_student, vblt_sexually_active, date_created, \n"
			...
			+ "                    sum(case\n" 
			+ "                        when sub_service_id in (214,235,236,237,238,239,240,241,242,243,244,245,246,247,248,249,250,251) then 1\n"
			+ "                        else 0\n" 
			...
			+ "                    max(case\n"
			+ "                        when sub_service_id in (214,235,236,237,238,239,240,241,242,243,244,245,246,247,248,249,250,251) then service_date\n"
			+ "                    end) approaches_date,"
	...
	public static final String SIMPLIFIED_AGYW_PREV = "select id, beneficiary_id, nui, current_age, date_of_birth, district_id, vblt_is_student, vblt_sexually_active, date_created, \n"
			...
			+ "                    end) mandatory_social_assets,\n"
			+ "                    sum(case\n"
			+ "                    when sub_service_id in (162) then 1\n"
			+ "                    else 0\n" 
			+ "                    end) mandatory_social_assetss,\n" 
			+ "                    sum(case\n"
			+ "                    when sub_service_id in (179,180,181,182,183) then 1\n"
			+ "                    else 0\n" 
			+ "                    end) mandatory_hiv_prevention,\n"
			+ "                    sum(case\n"
			+ "                    when sub_service_id in (196,198,199,200) then 1\n"
			+ "                    else 0\n" 
			+ "                    end) mandatory_violence_prevention,\n"
			+ "                    sum(case\n"
			...
			+ "                    end) guiao_hiv_sessions,\n"
			+ "                    sum(case\n"
			+ "                        when sub_service_id in (210,212) then 1\n"
			+ "                        else 0\n" 
			+ "                    end) guiao_gbv_sessions,\n"
			+ "                    sum(case\n" 
			+ "                        when sub_service_id in (216) then 1\n"
	...
	public static final String NEWLY_ENROLLED_AGYW_AND_SERVICES= ""
			...
			+ "		if (v.gender = 2 and v.current_age > 8 and v.vulnerable and community_interventions > 0,1,0) agyw_prev,\n"
			+ "		v.vblt_lives_with com_quem_mora,\n"
			+ "		if(v.vblt_idp=0,'Não', if(v.vblt_idp=1,'Sim','')) idp,\n"
			...
			+ "	left join us bus on v.beneficiary_us_id = bus.id\n"
	...
	public static final String NEWLY_ENROLLED_AGYW_AND_SERVICES_SUMMARY= ""
			...
			+ "		v.date_of_birth data_nascimento,\n"
			+ "		if (v.gender = 2 and v.current_age > 8 and v.vulnerable and community_interventions > 0,1,0) agyw_prev,\n"
			+ "	    coalesce(v.general_vulnerabilities,0) + coalesce(v.specific_vulnerabilities,0) as vulnerabilidades,\n"
			+ "		sum(case\n"
	...
	public static final String BENEFICIARIES_VULNERABILITIES = ""
			+ "select  provincia,\n"
			+ "	distrito,\n"
			+ "	onde_mora,\n"
			+ "	ponto_entrada,\n"
			...
			+ "	faixa_actual,\n"
			+ "	data_nascimento,\n"
			+ "	if(agyw_prev=1,'Sim','Não') vulneravel,\n"
			+ "	com_quem_mora,\n"
			+ "	idp,\n"
			+ "	sustenta_casa,\n"
			+ "	vai_escola,\n"
			...
			+ "	teste_hiv,\n"
			+ "	sexualmente_activa,\n"
			+ "	gravida_lactante_filhos,\n"
			+ "	vitima_exploracao_sexual_trafico,\n"
			+ "	trabalhadora_sexo,\n"
			+ "	relacoes_multiplas_concorrentes,\n"
			+ "	vitima_vbg,\n"
			+ "	tipo_vbg,\n"
			+ "	abuso_alcool_drogas,\n"
			+ "	historico_its\n"
			...
			+ "		if(v.vblt_married_before=0,'Não', if(v.vblt_married_before=1,'Sim','')) foi_casada,\n"
			+ "		if(v.vblt_tested_hiv=0,'Não', if(v.vblt_tested_hiv=1,'Sim (+3 meses)',if(v.vblt_tested_hiv=2,'Sim (-3 meses)',''))) teste_hiv,\n"
			+ "		if(v.vblt_sexually_active=0,'Não', if(v.vblt_sexually_active=1,'Sim','')) sexualmente_activa,\n"
			+ "		if(v.vblt_pregnant_or_has_children=0,'Não', if(v.vblt_pregnant_or_has_children=1,'Sim','')) gravida_lactante_filhos,\n"
			+ "		if(v.vblt_sexual_exploitation_trafficking_victim=0,'Não', if(v.vblt_sexual_exploitation_trafficking_victim=1,'Sim','')) vitima_exploracao_sexual_trafico,\n"
			+ "		if(v.vblt_sex_worker=0,'Não', if(v.vblt_sex_worker=1,'Sim','')) trabalhadora_sexo,\n"
			+ "		if(v.vblt_multiple_partners=0,'Não', if(v.vblt_multiple_partners=1,'Sim','')) relacoes_multiplas_concorrentes,\n"
			+ "		if(v.vblt_gbv_victim=0,'Não', if(v.vblt_gbv_victim=1,'Sim','')) vitima_vbg,\n"
			+ "		v.vblt_gbv_type tipo_vbg,\n"
			+ "		if(v.vblt_alcohol_drugs_use=0,'Não', if(v.vblt_alcohol_drugs_use=1,'Sim','')) abuso_alcool_drogas,\n"
			+ "		if(v.vblt_sti_history=0,'Não', if(v.vblt_sti_history=1,'Sim','')) historico_its\n"
			+ "	from\n"
			+ "	(\n"
			+ "		select beneficiary_id,max(service_date)\n"
			+ "		from agyw_prev_mview\n"
	...
	public static final String BENEFICIARIES_WITHOUT_SPECIFIC_VULNERABILITIES = ""
			...
			+ "     and (b.vblt_sexual_exploitation_trafficking_victim is null\n"
			+ "		and b.vblt_pregnant_or_has_children is null\n"
	...
	public static final String BENEFICIARIES_IN_WAITING_LIST = ""
			+ "select distinct pr.name provincia,\n"
			+ "	dt.name distrito,\n"
			+ "	n.name onde_mora,\n"
			+ "	if(b.entry_point=1,'US',if(b.entry_point=2,'CM','ES')) ponto_entrada,\n"
			...
			+ "	b.date_of_birth data_nascimento\n"
			+ "from beneficiaries b\n"
			+ "inner join agyw_prev_mview v on v.beneficiary_id = b.id\n"
			+ "left join district dt on b.district_id = dt.id\n"
			+ "left join province pr on dt.province_id = pr.id\n"
			+ "left join neighborhood n on b.neighbourhood_id = n.id\n"
			+ "left join partners pa on b.organization_id = pa.id\n"
